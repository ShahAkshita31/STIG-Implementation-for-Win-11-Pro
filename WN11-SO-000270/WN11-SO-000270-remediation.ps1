...
.USAGE
    ...
    Example syntax:
    PS C:\> .\WN11-SO-000270-remediation.ps1
#>
...
Write-Host "Starting Remediation for STIG ID: WN11-SO-000270" -ForegroundColor Cyan
Write-Host "Goal: Enable User Account Control (Admin Approval Mode)" -ForegroundColor Gray
Write-Host "------------------------------------------------------------" -ForegroundColor Cyan
...
try {
    ...
    if ($currentValue -eq $desiredValue) {
        ...
        if ($newValue -eq $desiredValue) {
            ...
            Write-Host "[INFO] A system restart is required for the change to take effect." -ForegroundColor Yellow
        }
        else {
